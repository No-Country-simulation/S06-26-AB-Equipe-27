    ...
    <style>
        ...
        * {
        ...
        .btn-setup:hover {
            ...
            box-shadow: 0 14px 26px -10px rgba(180, 83, 9, .65);
        }
        ...
        .score-fill-green {
            background: linear-gradient(90deg, var(--color-shield), #245f59);
        }
        ...
        .mini-goal-list::-webkit-scrollbar {
            width: 6px;
        }

        .mini-goal-list::-webkit-scrollbar-thumb {
        ...
        .mini-status.st-not-started {
            background-color: #F1F0F6;
            color: var(--color-muted);
        }

        .mini-status.st-in-progress {
            background-color: var(--level-senior-bg);
            color: var(--level-senior-fg);
        }

        .mini-status.st-completed {
            background-color: var(--color-shield-soft);
            color: var(--color-shield);
        }
        ...
        .table-skillfocus thead th:last-child {
            text-align: right;
        }
        ...
        .candidate-profile-row .circular-progress {
            position: relative;
            display: flex;
            align-items: center;
            justify-content: center;
        ...
        .candidate-profile-row .circular-progress svg {
            width: 100%;
            height: auto;
            transform: rotate(-90deg);
        }

        .candidate-profile-row .circular-progress circle {
            fill: none;
            stroke-width: 16;
            stroke-linecap: round;
        }

        .candidate-profile-row .circular-progress .circle-bg {
            stroke: #E9E5F3;
        }

        .candidate-profile-row .circular-progress .circle-progress {
            stroke-dasharray: 502.65;
            transition: stroke-dashoffset 0.5s ease;
        }

        .candidate-profile-row .circular-progress-value {
            font-family: var(--font-display);
        ...
        .candidate-profile-row .circular-progress-caption {
...
<body>
    ...
    <main class="container my-5">
        ...
        <div class="page-heading mb-4">
            <div class="eyebrow mb-2">Painel geral</div>
            <h1 class="mb-1">Olá, {{ auth()->user()->name }}!</h1>
            <p class="mb-0">Acompanhe suas candidaturas e encontre novas oportunidades</p>
        </div>

        {{-- Alerta de configuração pendente --}}
        @if(!$candidate->setup_completed)
        <div class="setup-alert d-flex flex-column flex-md-row align-items-md-center gap-3 mb-4">
            <div class="icon-wrap flex-shrink-0">
                <i class="bi bi-exclamation-triangle-fill"></i>
            </div>
            <div class="flex-grow-1">
                <h3 class="mb-1">Conclua seu perfil</h3>
                <p>Termine de configurar seu perfil para aumentar suas chances de encontrar a vaga perfeita</p>
            </div>
            <a href="{{ route('candidate-setup.step1') }}" class="btn-setup text-center flex-shrink-0">Completar perfil</a>
        </div>
        @endif
        ...
        <div class="row g-4 mb-4">
            <div class="col-12 col-md-4">
                <a href="{{ url('/jobs') }}" class="dash-card action-card">
                    ...
                    <div>
                        ...
                        <p>Explorar oportunidades disponíveis</p>
                    </div>
                    <i class="bi bi-arrow-right arrow-hint"></i>
                </a>
            </div>
            ...
            <div class="col-12 col-md-4">
                <a href="{{ url('/jobs?filter=applied') }}" class="dash-card action-card">
                    ...
                    <div>
                        ...
                        <p>Ver o status das suas aplicações</p>
                    </div>
                    <i class="bi bi-arrow-right arrow-hint"></i>
                </a>
            </div>
            ...
            <div class="col-12 col-md-4">
                <a href="{{ url('/candidate-setup') }}" class="dash-card action-card">
                    <div class="icon-box icon-blue"><i class="bi bi-person-gear"></i></div>
                    <div>
                        <h3>Editar Perfil</h3>
        ...
        <div class="row g-4 mb-4 align-items-stretch">
            {{-- Profile Progress --}}
            <div class="col-12 col-md-6 col-lg-4">
                <div class="dash-card h-100 candidate-stat-card">
                    <div class="widget-header">
                        <div class="icon-box icon-box-sm icon-purple"><i class="bi bi-person-check-fill"></i></div>
                        <div>
                            <h3>Progresso do Perfil</h3>
                            <p>Complete seu perfil para mais matches</p>
                        </div>
                    </div>
                    <div class="candidate-stat-body">
                        <div class="d-flex align-items-center gap-3 candidate-profile-row">
                            <div class="circular-progress">
                                <svg viewBox="0 0 180 180" aria-hidden="true">
                                    ...
                                    <circle
                                        ...
                                        cy="90"
                                        r="74"
                                        style="stroke: url(#profileProgressGradient); stroke-dashoffset: {{ 502.65 - (502.65 * $profileProgress / 100) }};">
                                    </circle>
                                    <defs>
                                        <linearGradient id="profileProgressGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                                            <stop offset="0%" stop-color="#7C3AED" />
                                            <stop offset="100%" stop-color="#4e1ba5" />
                                        </linearGradient>
                                    </defs>
                                </svg>
                                <div class="position-absolute top-50 start-50 translate-middle text-center">
                                    <span class="circular-progress-value">{{ $profileProgress }}%</span>
                                    <div class="circular-progress-caption">de progresso</div>
                                </div>
                            </div>
                            <div class="flex-grow-1">
                                <ul class="list-unstyled candidate-checklist mb-0">
                                    <li>
                                        <i class="bi {{ $profileFields['resume'] ? 'bi-check2 text-success' : 'bi-x text-muted' }}"></i>
                                        <span>Currículo</span>
                                    </li>
                                    <li>
                                        <i class="bi {{ $profileFields['languages'] ? 'bi-check2 text-success' : 'bi-x text-muted' }}"></i>
                                        <span>Idiomas</span>
                                    </li>
                                    <li>
                                        <i class="bi {{ $profileFields['portfolio'] ? 'bi-check2 text-success' : 'bi-x text-muted' }}"></i>
                                        <span>Portfólio</span>
                                    </li>
                                    <li>
                                        <i class="bi {{ $profileFields['salary'] ? 'bi-check2 text-success' : 'bi-x text-muted' }}"></i>
                                        <span>Pretensão Salarial</span>
                                    </li>
                                </ul>
                                @if($profileProgress < 100)
                                    <a href="{{ route('candidate-setup.step1') }}" class="btn btn-apply mt-3 d-inline-block">Completar Perfil</a>
                                    @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="dash-card h-100 score-card">
                    <div class="widget-header">
                        <div class="icon-box icon-box-sm icon-green"><i class="bi bi-stars"></i></div>
                        <div>
                            <h3>Score de Match</h3>
                            <p>Compatibilidade com vagas atuais</p>
                        </div>
                    </div>
                    <div class="flex-grow-1 d-flex flex-column justify-content-center text-center">
                        <div>
                            <span class="score-value score-green">{{ $matchScore }}</span><span class="score-max">%</span>
                        </div>
                        <div class="score-track">
                            <div class="score-fill score-fill-green" style="width: {{ min($matchScore, 100) }}%"></div>
                        </div>
                        <span class="score-caption">Seu perfil é compatível com as vagas atuais</span>
                        <span class="candidate-match-badge mt-2">+{{ rand(5, 15) }} novas vagas hoje</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12 col-md-6 col-lg-4">
                <div class="dash-card h-100 candidate-stat-card">
                    <div class="widget-header">
                        <div class="icon-box icon-box-sm icon-amber"><i class="bi bi-file-earmark-text-fill"></i></div>
                        <div>
                            <h3>Saúde do Currículo</h3>
                            <p>Qualidade e completude do CV</p>
                        </div>
                    </div>
                    <div class="candidate-stat-body">
                        <div>
                            <span class="score-value score-amber">88</span><span class="score-max">/100</span>
                            <ul class="list-unstyled candidate-checklist mt-3 mb-0">
                                <li>
                                    <i class="bi bi-exclamation-triangle" style="color: var(--color-shield-warn);"></i>
                ...
                <div class="dash-card table-widget h-100">
                    <div class="widget-header">
                        ...
                        <div>
                            ...
                            <p>Acompanhe o status das suas aplicações</p>
                        </div>
                    </div>
                    <div class="table-skillfocus-wrap mb-4">
                        <table class="table-skillfocus">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $app->jobPosting?->title ?? 'Vaga' }}</td>
                                    <td>{{ $app->company?->name ?? 'Empresa' }}</td>
                                    <td>
                                        <span class="status-pill {{ $app->status === 'selecionado' ? 'selected' : ($app->status === 'pendente' ? 'pending' : 'reviewed') }}">
                                            {{ ucfirst($app->status) }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-end">
                                        @if($app->jobPosting)
                                        <a href="{{ route('jobs.show', $app->jobPosting) }}" class="btn-ver-vagas d-inline-block" title="Ver detalhes">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        @endif
                                    </td>
                ...
                <div class="dash-card h-100">
                    <div class="widget-header">
                        ...
                        <div>
                            ...
                            <p>Oportunidades selecionadas para você</p>
                        </div>
                    </div>
                    <div class="row g-3">
                        @foreach($jobs as $job)
                                ...
                                <div class="d-flex align-items-center gap-2 mb-2">
                                    ...
                                    <div class="flex-grow-1 min-w-0">
                                        ...
                                        <div class="d-flex align-items-center flex-wrap gap-2 mt-1">
                                            ...
                                            <span class="candidate-job-meta"><i class="bi bi-geo-alt-fill"></i> {{ $job->city }}</span>
                                        </div>
                                    </div>
                                </div>
                                @if(in_array($job->id, $appliedJobIds))
                                <a href="{{ route('jobs.show', $job) }}" class="btn btn-apply w-100 text-decoration-none">Ver vaga</a>
                                @else
                                <form action="{{ route('jobs.apply', $job) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-apply w-100">Aplicar</button>
                                </form>
                                @endif
                            </div>
                        </div>
    ...
    <footer class="container pb-4 pt-2">
        <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-2 pt-3">
            <span>© 2026 SkillFocus — {{ $isCompanyUser ? 'Plataforma de RH com foco em diversidade' : 'Plataforma de Candidato com foco em diversidade' }}</span>
